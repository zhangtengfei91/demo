...
import cn.edu.shou.missive.service.FaxCableRepository;
import cn.edu.shou.missive.service.FullSearchService;
...
public class FullSearchController {
    ...
    @Autowired
    FullSearchService searchR;
    @Autowired
    ActivitiService act;
    @Autowired
    FaxCableRepository faxR;
    @Autowired
    UserRepository usrR;
    @Autowired
    ...
    public String setSearchContent(@PathVariable("taskType")String taskType,@PathVariable("taskID")String taskID,@PathVariable("processID")String processID,@AuthenticationPrincipal User currentUser){
        ...
        if (isFishedProcess) {
            fullsearchR.setSearchContent(taskType, taskID, processID);
        }
//        //ID为公文记录对应的编号
